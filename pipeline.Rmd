...
Validation: Abundance Check for High-FC Fungi

```{r}
# ============================================================
# CRITICAL VALIDATION: Are DESeq2 results real or artifacts?
# ============================================================

cat("\n\n=== ABUNDANCE VALIDATION FOR HIGH log2FC FUNGI ===\n\n")

# Get rarefied OTU table (after normalization to 305 reads)
otu_fungi_rarefied <- as.data.frame(otu_table(ps_rarefied_its))
meta_fungi <- data.frame(sample_data(ps_rarefied_its))
tax_its <- as.data.frame(tax_table(ps_rarefied_its))

sarc_samples <- rownames(meta_fungi)[meta_fungi$Group == "Sarcoidosis"]
old_samples <- rownames(meta_fungi)[meta_fungi$Group == "Other_Lung_Diseases"]

# Focus on top candidates from DESeq2
candidates <- c("Cladosporium", "Rozellomycota_gen_Incertae_sedis")

for(taxon_name in candidates) {
  cat("\n=== ", taxon_name, " ===\n", sep="")
  
  # Find index in taxonomy
  idx <- which(grepl(taxon_name, tax_its$Genus, ignore.case=TRUE))[1]
  
  if(!is.na(idx)) {
    
    sarc_values <- as.numeric(otu_fungi_rarefied[idx, sarc_samples])
    old_values <- as.numeric(otu_fungi_rarefied[idx, old_samples])
    
    cat("After rarefaction (305 reads/sample):\n\n")
    
    cat("SARCOIDOSIS (n=", length(sarc_samples), "):\n", sep="")
    cat("  Reads:", paste(round(sarc_values, 1), collapse=", "), "\n")
    cat("  Sum:", sum(sarc_values), "\n")
    cat("  Mean:", round(mean(sarc_values), 2), "\n")
    cat("  Median:", round(median(sarc_values), 2), "\n")
    cat("  % of total 305 reads:", round(mean(sarc_values)/305*100, 2), "%\n")
    cat("  Prevalence:", sum(sarc_values > 0), "/", length(sarc_samples), "\n\n")
    
    cat("OTHER LUNG DISEASES (n=", length(old_samples), "):\n", sep="")
    cat("  Reads:", paste(round(old_values, 1), collapse=", "), "\n")
    cat("  Sum:", sum(old_values), "\n")
    cat("  Mean:", round(mean(old_values), 2), "\n")
    cat("  Median:", round(median(old_values), 2), "\n")
    cat("  % of total 305 reads:", round(mean(old_values)/305*100, 2), "%\n")
    cat("  Prevalence:", sum(old_values > 0), "/", length(old_samples), "\n\n")
    
    cat("ASSESSMENT:\n")
    if(mean(sarc_values) < 0.1 && mean(old_values) < 0.1) {
      cat("  ⚠️ BOTH groups have extremely low abundance (<0.1 reads)\n")
      cat("     Signal is at NOISE level, likely ARTIFACT\n")
    } else if(mean(sarc_values) < 1) {
      cat("  ⚠️ Mean abundance in Sarcoidosis <1 read\n")
      cat("     After rarefaction to 305 reads, this is <0.3%\n")
      cat("     Signal may be lost in statistical noise\n")
    } else {
      cat("  ✓ Mean abundance in Sarcoidosis ≥1 read\n")
      cat("     More robust for analysis\n")
    }
    
    # Wilcoxon on these specific values
    wtest <- wilcox.test(sarc_values, old_values)
    cat("  Wilcoxon p-value:", format(wtest$p.value, digits=4), "\n")
    
  } else {
    cat("  ERROR: Not found in taxonomy\n")
  }
}

cat("\n\n=== OVERALL ASSESSMENT ===\n")
cat("If high log2FC fungi have <0.5 reads mean abundance in rarefied data:\n")
cat("  → Results are HYPOTHESIS-GENERATING ONLY\n")
cat("  → Must validate with qPCR on original samples\n")
cat("  → Or increase sequencing depth for fungi (currently 305 reads is very low)\n")
```
